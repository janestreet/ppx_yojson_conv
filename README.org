...
into scope in order for the ppx to use them, for example by adding
=open Ppx_yojson_conv_lib.Yojson_conv.Primitives= at the beginning of the file.
...
#+begin_src ocaml
  type t =
    { a : u [@default u0] [@yojson_drop_default (=)]; (* explicit user-provided function *)
      ...
      f : u [@default u0] [@yojson_drop_default.compare.local]; (* uses [%compare.equal__local: u] *)
      g : u [@default u0] [@yojson_drop_default.equal.local]; (* uses [%equal__local: u] *)
    } [@@deriving yojson]
#+end_src
